...
const FORGE_INSTALLER_URLS = [
  ...
  `https://bmclapi2.bangbang93.com/maven/${FORGE_INSTALLER_PATH}`,
];
...
function buildServersDat(servers) {
  ...
  const p = [];
  p.push(Buffer.from([0x0a]), nbtString(""));                
  p.push(Buffer.from([0x09]), nbtString("servers"));         
  p.push(Buffer.from([0x0a]), int32(servers.length));        
...
function findSystemJava() {
  ...
  for (const c of candidates) {
    try {
      if (String(c).toLowerCase().includes("windowsapps")) continue;
      let bin = null;
...
ipcMain.on("save-state", (_e, data) => { try { mkdirp(ROOT).then(() => writeState({ ui: data })); } catch {  } });
