...
export default function App() {
  ...
  const play = useCallback(async () => {
    ...
    toast("ok", `Запускаю Minecraft от имени ${selected.username}…`);
    try {
      const res = await api.launchGame(settings, selected);
      if (!res.ok) toast("err", res.message ?? "Не удалось запустить игру");
      else toast("ok", "Игра запущена. Удачной охоты!");
    } catch (e) {
